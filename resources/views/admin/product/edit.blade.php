        ...
        <div class="mb-3">
            ...
            @if($product->images && count($product->images) > 0)
                @foreach ($product->images as $item)
                    <a href="{{ route('admin.delete_img.product', $item->id) }}" class="text-danger"> X </a>
                    <img src="{{ asset('images/' . $item->image) }}" width="100px" alt="Additional Image">
                @endforeach
            @else
            ...
            <div id="variant-container">
                @foreach ($product->variants as $index => $variant)
                    <div class="variant-item mt-2">
                        <input type="text" name="variants[{{ $index }}][name]" class="form-control" value="{{ old("variants.$index.name", $variant->name) }}" >
                        @error("variants.$index.name")
                            <span class="text-danger">{{ $message }}</span>
                        @enderror

                        <input type="number" name="variants[{{ $index }}][price]" class="form-control mt-2" value="{{ old("variants.$index.price", $variant->price) }}" >
